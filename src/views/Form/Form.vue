<template>
  <div class="form">
    <el-form :model="form" :rules="rules" ref="ruleForm" label-width="120px">
      ...
      <el-form-item label="性别" prop="people1.sex" :rules="getRule('性别')">
        <el-input v-model="form.people1.sex"></el-input>
      </el-form-item>
      ...
      <el-form-item label="性别" prop="people2.sex" :rules="getRule('性别')">
...
export default {
  ...
  methods: {
    getRule (prop) {
      return [{ required: true, message: `${prop}不能为空`, trigger: 'blur' }]
    },
    submitForm (formName) {
  ...
  created () {
    const discription = `
    <pre> 
      问题描述: form数据结构嵌套的表单校验 实现行内校验或者动态校验 
      例如form的结构为：
      form: {
        class: '',
        people1: {
